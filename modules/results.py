...
def key_generate_filter(dictionary, keys_list, filter_off, filtered_value):
    ...
    keys_list  | If equals True then autogenerate keys, else will pass the existing list
    filter_off  | 1 equals off, 0 equals on. Only filters autogenerated keys.
    filtered_value | key to be filtered. ## Typically 'ALL'
    ...
    # Generates keys_list with all values when keys_list is True
    if keys_list is True:
        keys = list(dictionary.keys())
        if filter_off == 0:
            if filtered_value in keys:
                keys.remove(filtered_value)
    # Generates keys_list with only filtered value when false.
    elif keys_list is False:
        keys = [filtered_value]
    return keys
...
def key_all_expand(dictionary, key):
    ...
    if key is True:
        ...
        keys_list = key_generate_filter(dictionary, True, 0, 'ALL')
    elif key is False:
        key_includes_all = False
        keys_list = key_generate_filter(dictionary, False, 0, 'ALL')
...
def x_y_labels_generate_flat(stats, i_include=True, j_include=False, a_include=False, r_include=False, d_include=False, c_include=False, s_include=False, filter_t=False):
    """
    x_y_labels_generate_flat()
    ...
    Use note: With current implementation there are no 'ALL' values for i.
    ...
    for key, time_dict in stats.items():
...
def key_include_filter(key,index,_include=False):
    ...
    elif type(_include) is list:
        if key[index] in _include:
            return True
        else:
            return False
    else:
        print('Argument _include passed to key_include_filter() was not a list or boolean.\n_include Type: '+str(type(_include())+'\nValue: '+str(_include)))
...
def x_y_labels_generate(stats, i, j, a, r, d, c, s, t_keys, labels_on=[0, 0, 0, 0, 0, 0, 0], y=None, labels=None, filter_value=False, filter_pop=True):
    ...
    if y[-1] == []:
        ...
        # potential simplication, although not implemented due to flexibility
        # for ind, l_string in enumerate([i,j,a,r,d,c,s]):
        #    if labels_on[ind] == 1:
        #        l_current += str(l_string + ' ')
        #
        l_current = ''
