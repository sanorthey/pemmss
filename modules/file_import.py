...
def import_static_files(path, copy_path_folder=None, log_file=None):
    ...
    static_files['imported_shapefile_gdf'] = import_shapefile(path / 'shapefile' / 'shapefile.shp', copy_path=copy_path_folder / 'shapefile', log_path=log_file)
    # TODO: import geopackage instead of shapefile due to data format limitations of shapefiles
...
def import_projects(f, path, shapefile_gdf=None, copy_path=None, log_path=None):
    ...
    with open(path, mode='r') as input_file:
        ...
        for row in csv_reader:
            ...
            if row['LATITUDE'] == "" or row['LONGITUDE'] == "":
                ...
                if shapefile_gdf is not None:  # If latitude or longitude is missing then throw out the other (only makes sense if they are a pair) and generate a new coordinate from scratch.
                    latitude, longitude = spatial.generate_region_coordinate(shapefile_gdf, 'REGION', region, method='random')
                else:
    ...
    if log_path is not None:
        ...
        export_log(str(no_latitude) + ' : latitude. Missing coordinate randomly generated in generated region if it is in the shapefile.',output_path=log_path)
        export_log(str(no_longitude) + ' : longitude. Missing coordinate randomly generated in generated regionif it is in the shapefile.',output_path=log_path)
...
def import_shapefile(path, copy_path=None, log_path=None):
    """
        import_shapefile()
        Imports a shapefile as a geopandas dataframe.
        Copies shapefile.shp if copy_path directory specified.
        ...
        """
    if path.exists():
        shapefile_gdf = spatial.import_shapefile(path)
        log_message = 'Imported shapefile'
        if copy_path is not None:
            copyfile(path, copy_path / 'shapefile.shp')
    else:
        shapefile_gdf = None
        log_message = "Warning: Shapefile not found. Missing coordinates or coordinates for greenfield deposits will not be generated."
